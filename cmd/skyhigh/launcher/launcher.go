...
import (
	...
	"github.com/skyhighblockchain/skyhigh/cmd/skyhigh/launcher/metrics"
	"github.com/skyhighblockchain/skyhigh/cmd/skyhigh/launcher/tracing"
	"github.com/skyhighblockchain/skyhigh/debug"
	"github.com/skyhighblockchain/skyhigh/flags"
	"github.com/skyhighblockchain/skyhigh/gossip"
	"github.com/skyhighblockchain/skyhigh/integration"
	"github.com/skyhighblockchain/skyhigh/utils/errlock"
	"github.com/skyhighblockchain/skyhigh/valkeystore"
	_ "github.com/skyhighblockchain/skyhigh/version"
)

const (
	// clientIdentifier to advertise over the network.
	clientIdentifier = "skyhigh"
)
...
var (
	...
	app = flags.NewApp(gitCommit, gitDate, "the skyhigh command line interface")
...
func initFlags() {
	...
	nodeFlags = append(nodeFlags, skyhighFlags...)
...
// skyhigh is the main entry point into the system if no special subcommand is ran.
// It creates a default node based on the command line arguments and runs it in
// blocking mode, waiting for it to be shut down.
func pushMain(ctx *cli.Context) error {
	...
	genesisPath := getSkyhighGenesis(ctx)
...
func makeNode(ctx *cli.Context, cfg *config, genesis integration.InputGenesis) (*node.Node, *gossip.Service, func()) {
	...
	valPubkey := cfg.Skyhigh.Emitter.Validator.PubKey
	if key := getFakeValidatorKey(ctx); key != nil && cfg.Skyhigh.Emitter.Validator.ID != 0 {
	...
	svc, err := gossip.NewService(stack, cfg.Skyhigh, gdb, signer, blockProc, engine, dagIndex)
	if err != nil {
...
func startNode(ctx *cli.Context, stack *node.Node) {
	...
	// Create a client to interact with local skyhigh node.
	...
	if ctx.GlobalBool(utils.ExitWhenSyncedFlag.Name) {
		go func() {
			for first := true; ; first = false {
				// Call skh_syncing until it returns false
				time.Sleep(5 * time.Second)

				var syncing bool
				err := rpcClient.CallContext(context.TODO(), &syncing, "skh_syncing")
				if err != nil {
					continue
				}
				if !syncing {
