...
import (
	...
	"github.com/skyhighblockchain/skyhigh/integration"
	...
	futils "github.com/skyhighblockchain/skyhigh/utils"
...
var (
	...
	// DataDirFlag defines directory to store Push state and user's wallets
	...
	RPCGlobalGasCapFlag = cli.Uint64Flag{
		Name:  "rpc.gascap",
		Usage: "Sets a cap on gas that can be used in skh_call/estimateGas (0=infinite)",
		Value: gossip.DefaultConfig(cachescale.Identity).RPCGasCap,
	}
	RPCGlobalTxFeeCapFlag = cli.Float64Flag{
		Name:  "rpc.txfeecap",
		Usage: "Sets a cap on transaction fee (in SKH) that can be sent via the RPC APIs (0 = no cap)",
		Value: gossip.DefaultConfig(cachescale.Identity).RPCTxFeeCap,
	}

	AllowedSkyhighGenesisHashes = map[uint64]hash.Hash{
		skyhigh.MainNetworkID: hash.HexToHash("0x8895b98d25c653773a31be420a6a29d322a10107e76dff37dc694ad02ebacd01"),
		skyhigh.TestNetworkID: hash.HexToHash("0xc4a5fc96e575a16a9a0c7349d44dc4d0f602a54e0a8543360c2fee4c3937b49e"),
	}
)
...
type config struct {
	Node         node.Config
	Skyhigh      gossip.Config
	SkyhighStore gossip.StoreConfig
	Push         abft.Config
	PushStore    abft.StoreConfig
...
func (c *config) AppConfigs() integration.Configs {
	return integration.Configs{
		Skyhigh:        c.Skyhigh,
		SkyhighStore:   c.SkyhighStore,
		Push:           c.Push,
		PushStore:      c.PushStore,
		VectorClock:    c.VectorClock,
		AllowedGenesis: AllowedSkyhighGenesisHashes,
	}
}
...
func getSkyhighGenesis(ctx *cli.Context) integration.InputGenesis {
	...
	case ctx.GlobalIsSet(FakeNetFlag.Name):
		...
		fakeGenesisStore := makegenesis.FakeGenesisStore(num, futils.ToSkh(1000000000), futils.ToSkh(5000000))
...
func mayMakeAllConfigs(ctx *cli.Context) (*config, error) {
	...
	cfg := config{
		...
		Skyhigh:      gossip.DefaultConfig(cacheRatio),
		SkyhighStore: gossip.DefaultStoreConfig(cacheRatio),
		Push:         abft.DefaultConfig(),
		PushStore:    abft.DefaultStoreConfig(cacheRatio),
		VectorClock:  vecmt.DefaultConfig(cacheRatio),
	}

	if ctx.GlobalIsSet(FakeNetFlag.Name) {
		_, num, _ := parseFakeGen(ctx.GlobalString(FakeNetFlag.Name))
		cfg.Skyhigh = gossip.FakeConfig(num, cacheRatio)
	}
	...
	var err error
	cfg.Skyhigh, err = gossipConfigWithFlags(ctx, cfg.Skyhigh)
	if err != nil {
		return nil, err
	}
	cfg.SkyhighStore, err = gossipStoreConfigWithFlags(ctx, cfg.SkyhighStore)
	if err != nil {
		return nil, err
	}
	cfg.Node = nodeConfigWithFlags(ctx, cfg.Node)
	if cfg.Skyhigh.Emitter.Validator.ID != 0 && len(cfg.Skyhigh.Emitter.PrevEmittedEventFile.Path) == 0 {
		cfg.Skyhigh.Emitter.PrevEmittedEventFile.Path = cfg.Node.ResolvePath(path.Join("emitter", fmt.Sprintf("last-%d", cfg.Skyhigh.Emitter.Validator.ID)))
	}

	if err := cfg.Skyhigh.Validate(); err != nil {
		return nil, err
	}
...
func defaultNodeConfig() node.Config {
	...
	cfg.HTTPModules = append(cfg.HTTPModules, "eth", "skh", "dag", "sfc", "abft", "web3")
	cfg.WSModules = append(cfg.WSModules, "eth", "skh", "dag", "sfc", "abft", "web3")
	cfg.IPCPath = "skyhigh.ipc"
	cfg.DataDir = DefaultDataDir()
	return cfg
}
