...
import (
	...
	"github.com/skyhighblockchain/skyhigh/cmd/cmdtest"
)

func tmpdir(t *testing.T) string {
	dir, err := ioutil.TempDir("", "skyhigh-test")
	if err != nil {
		t.Fatal(err)
	}
	return dir
...
func init() {
	// Run the app if we've been exec'd as "skyhigh-test" in exec().
	reexec.Register("skyhigh-test", func() {
...
func exec(t *testing.T, args ...string) *testcli {
	...
	// Boot "skyhigh". This actually runs the test binary but the TestMain
	// function will prevent any tests from running.
	tt.Run("skyhigh-test", args...)
