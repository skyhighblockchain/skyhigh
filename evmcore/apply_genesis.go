...
func ApplyGenesis(statedb *state.StateDB, g skyhigh.Genesis, maxMemoryUsage int) (*EvmBlock, error) {
	mem := 0
...
func genesisBlock(g skyhigh.Genesis, root common.Hash) *EvmBlock {
	block := &EvmBlock{
		EvmHeader: EvmHeader{
			Number:   big.NewInt(0),
			Time:     g.Time,
...
func MustApplyGenesis(g skyhigh.Genesis, statedb *state.StateDB, maxMemoryUsage int) *EvmBlock {
