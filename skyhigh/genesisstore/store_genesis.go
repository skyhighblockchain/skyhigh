...
func (s *Store) GetRules() skyhigh.Rules {
	cfg := s.rlp.Get(s.table.Rules, []byte("c"), &skyhigh.Rules{}).(*skyhigh.Rules)
	return *cfg
}

func (s *Store) SetRules(cfg skyhigh.Rules) {
	s.rlp.Set(s.table.Rules, []byte("c"), &cfg)
}

func (s *Store) GetGenesis() skyhigh.Genesis {
	meatadata := s.GetMetadata()
	return skyhigh.Genesis{
