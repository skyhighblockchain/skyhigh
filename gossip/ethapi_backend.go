...
import (
	...
	"github.com/skyhighblockchain/skyhigh/topicsdb"
	"github.com/skyhighblockchain/skyhigh/tracing"
)
...
// GetEventPayload returns Push event by hash or short ID.
...
// GetEvent returns the Push event header by hash or short ID.
...
func (b *EthAPIBackend) GetEVM(ctx context.Context, msg evmcore.Message, state *state.StateDB, header *evmcore.EvmHeader, vmConfig *vm.Config) (*vm.EVM, func() error, error) {
	...
	if vmConfig == nil {
		vmConfig = &skyhigh.DefaultVMConfig
	}
